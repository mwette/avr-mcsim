...
\begin{tikzpicture}[%
  ...
  \node (WTS) [draw, rounded corners=15pt,
        ...
        at ($0.6*(RL)+0.4*(RC)$) 
        {\parbox[c]{1.0in}{\centering{WAIT FOR DATA SENT}}};

  \draw[->] (RD1) -- (RL) -- node[above] {11:[OK]/ACK} (WTS);
  ...
  % put zero out
  \draw[->] (RD4) -- node[above] {21:[n=0]/zero} (RD4 -| WST) -- (WST);
  ...
  \node (WFI) [draw, rounded corners=15pt,
               minimum height=0.4in, minimum width=0.75in, thick]
        at ($0.6*(WL)+0.4*(WC)$) 
        {\parbox[c]{1.0in}{\centering{WAIT FOR DATA RCVD}}};
